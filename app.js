  ...
  fetch('performance.json?v=' + Date.now())
    ...
    .then(function(p) {
      ...
      if (!p || p.bettingDays === 0) {
        ...
        if (label) label.textContent = '📊 Proof tracking starts from 23 May 2026 reset';
...
function renderProofHistory(days) {
  ...
  html += 'Every official result is tracked here from the 23 May 2026 reset, including losses.';
  html += '</div></div>';
