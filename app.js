...
function renderProofHistory(days) {
  ...
  if (!log || log.length === 0) {
    wrap.innerHTML = '<div style="text-align:center;padding:24px;font-family:'DM Mono',monospace;font-size:11px;color:var(--muted)">No selection history yet. Check back after today's results.</div>';
    return;
  }
  var html = '';
  log.forEach(function(day) {
    ...
    html += '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">'
      ...
      +'</div></div>';
    function renderSels(selList) {
      var h = '';
      selList.forEach(function(s) {
        var hc = s.result==='WON' ? 'var(--green)' : s.result==='PLACED' ? 'var(--gold)' : s.result==='PENDING'||!s.result ? 'var(--muted)' : 'var(--muted2)';
        var hi = s.result==='WON' ? '&#x2705;' : s.result==='PLACED' ? '&#x1F7E1;' : s.result==='VOID' ? '&#x26AA;' : s.result==='PENDING'||!s.result ? '&#x23F3;' : '&#x274C;';
        var oddsStr = s.odds ? decToFrac(s.odds) : '-';
        var posStr = s.position && s.position > 0 ? ' ('+s.position+')' : '';
    ...
    if (flatSels.length > 0) {
      html += '<div style="font-family:'DM Mono',monospace;font-size:8px;color:var(--gold);letter-spacing:.1em;text-transform:uppercase;margin-bottom:4px;margin-top:4px">Flat</div>';
      html += '<div style="border-top:1px solid rgba(255,255,255,0.05);padding-top:6px;margin-bottom:6px">'+renderSels(flatSels)+'</div>';
    }
    if (jumpsSels.length > 0) {
      html += '<div style="font-family:'DM Mono',monospace;font-size:8px;color:#7eb8f0;letter-spacing:.1em;text-transform:uppercase;margin-bottom:4px;margin-top:4px">Jumps</div>';
      html += '<div style="border-top:1px solid rgba(255,255,255,0.05);padding-top:6px">'+renderSels(jumpsSels)+'</div>';
    }
    html += '</div>';
  });
