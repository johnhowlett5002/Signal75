...
def build_runner_context(
    ...
    supplied_days_since = _positive_int(runner.get("days_since") or runner.get("daysSinceLastRun"))
    if supplied_days_since is not None:
        days_since_last_run = supplied_days_since
        latest_run_date = (date.fromisoformat(race_date) - timedelta(days=supplied_days_since)).isoformat()
        latest_run_source = "current_racecard"
    else:
        try:
            days_since_last_run = (date.fromisoformat(race_date) - date.fromisoformat(str(latest_run_date))).days
        except (TypeError, ValueError):
            days_since_last_run = None
        latest_run_source = "form_history.sqlite" if latest_run_date else None
    return {
        ...
        "latestRunSource": latest_run_source,
        ...
        "rpr": card.get("rpr"),
        "raceClass": runner.get("race_class") or card.get("race_class") or None,
