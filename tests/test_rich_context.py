...
def test_live_days_since_overrides_stale_archive_recency(tmp_path):
    module = load_module()
    database = tmp_path / "form.sqlite"
    seed_database(database)
    ...
    context = module.build_runner_context(
        database,
        {"name": "Merry", "venue": "Ripon", "race_name": "1m2f Hcap", "days_since": "18"},
        "2026-09-01",
    )

    assert context["daysSinceLastRun"] == 18
    assert context["latestRunDate"] == "2026-08-14"
    assert context["latestRunSource"] == "current_racecard"
...
def test_generator_no_longer_hardcodes_missing_rich_context_as_zero():
    ...
    spec = importlib.util.spec_from_file_location(
        "pick_generator_score_audit",
        ROOT / "scripts" / "generate-picks-betfair.py",
    )
    generator = importlib.util.module_from_spec(spec)
    assert spec.loader
    spec.loader.exec_module(generator)
    adjustments = [
        {"type": "bonus", "points": 3, "reason": "rich-form pattern"},
        {"type": "penalty", "points": 4, "reason": "class rise without stored proof"},
    ]
    pick = {
        "name": "Example Horse",
        "jockey": "A Jockey",
        ...
        "form": "123",
        "score": 86.0,
        "live_adjusted_score": 85.0,
        "official_score_adjustments": adjustments,
        "consensus": {},
        "venue": "York",
        "race_type": "flat",
        "race_name": "Example Stakes 1m",
        "race_time": "14:00",
        "field_size": 10,
    }

    horse = generator.build_race_entry(pick, "Test explanation")["horses"][0]

    assert horse["rawSignalScore"] == 86.0
    assert horse["officialAdjustedScore"] == 85.0
    assert horse["scoreAdjustments"] == adjustments
