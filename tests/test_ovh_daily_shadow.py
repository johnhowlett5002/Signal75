...
def test_candidate_launch_agent_prepares_inputs_before_morning_pipeline():
    path = ROOT / "deploy" / "launchd" / "co.signal75.ovh-candidate.plist"
    with path.open("rb") as handle:
        payload = plistlib.load(handle)

    assert payload["Label"] == "co.signal75.ovh-candidate"
    assert payload["StartCalendarInterval"] == {"Hour": 8, "Minute": 30}
    assert payload["ProgramArguments"][-1].endswith("ensure-ovh-shadow-candidate.sh")
    assert "RunAtLoad" not in payload
    assert "KeepAlive" not in payload
